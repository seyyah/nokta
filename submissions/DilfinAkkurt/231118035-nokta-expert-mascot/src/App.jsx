...
const S = {
  ...
  expertBubble: {
    alignSelf: 'flex-start', background: 'linear-gradient(135deg, #a855f7 0%, #7e22ce 100%)', color: '#ffffff',
    padding: '12px 16px', borderRadius: '20px 20px 20px 4px',
    fontSize: 15, maxWidth: '85%', lineHeight: 1.45,
...
export default function App() {
  ...
  const onTranscript = useCallback(async (transcript) => {
    ...
    try {
      ...
      setChat(p => [...p, { role: isExpertMode ? 'expert' : 'ai', text: reply }]);
      setStatus(voiceModeRef.current ? 'Konuşuyor...' : 'Konuşmak için mikrofona basın');
    } catch(err) {
      console.error('Brain error:', err);
      setStatus('Bağlantı hatası oluştu');
      setChat(p => [...p, { role: isExpertMode ? 'expert' : 'ai', text: 'Üzgünüm, bir sorun oluştu.' }]);
    } finally {
      setTyping(false);
    }
  }, [isExpertMode]);
  ...
  const toggleExpertMode = () => {
    ...
    Brain.setExpertMode(newMode);
    setShowChat(true);
    if (newMode) {
      setChat(p => [...p, { role: 'ai', text: "Sizi hemen uzmanımız Dilfin'e bağlıyorum, lütfen bekleyin..." }]);
      setTyping(true);
      setTimeout(() => {
        setTyping(false);
        setChat(p => [...p, { role: 'expert', text: 'Merhaba! Ben Dilfin, Canlı Uzman Desteği. Size nasıl yardımcı olabilirim?' }]);
      }, 2000);
    } else {
      setChat(p => [...p, { role: 'ai', text: 'Uzman desteğinden ayrıldınız. Artık ben (Nokta) size yardımcı olacağım.' }]);
    }
  };
      ...
      <div style={S.root}>
          ...
          <div ref={chatRef} style={S.chat}>
            {chat.map((m, i) => (
              <div key={i} style={m.role === 'user' ? S.userBubble : (m.role === 'expert' ? S.expertBubble : S.aiBubble)}>
                {m.role === 'expert' && <div style={{ fontSize: 10, opacity: 0.8, marginBottom: 4 }}>Canlı Uzman (Dilfin)</div>}
                {m.text}
              </div>
            ))}
        ...
        <div style={{ ...S.floatingBar, transform: showChat ? 'translateY(10px)' : 'translateY(0)' }}>
          ...
          <div style={S.textCol} onClick={() => setShowChat(false)}>
            <span style={{ ...S.subtitle, color: isExpertMode ? '#9333ea' : S.subtitle.color }}>
              {isExpertMode ? 'Canlı Uzman: Dilfin' : 'Sesli Asistan'}
            </span>
          ...
          <div style={S.iconBtnRow}>
            <button 
              style={{ ...S.iconBtn, background: isExpertMode ? '#f3e8ff' : '#ffffff', borderColor: isExpertMode ? '#d8b4fe' : '#f3f4f6' }} 
