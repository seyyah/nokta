...
3. **Veritabanı ve Performans:** Real-time senkronizasyon ve çevrimdışı destek için **Google Cloud Firestore** mimarisi seçildi. Konum tabanlı (Geo-query) sorgular için gerekli indeksleme yapılandırması yapılarak performans optimizasyonu sağlandı. Ek olarak, uzman modülü için `experts` koleksiyonu ve `needsExpertReview` alanları planlandı.
...
5. **Human Support (Uzman Desteği) Katmanı:** Sadece gönüllülerle çözülemeyecek medikal veya psikolojik destek ihtiyaçlarını karşılamak üzere projeye bir 'Uzman' (Doktor, Sosyal Hizmet Uzmanı vb.) rolü eklendi. Groq (Llama-3-70b) kullanılarak yaşlıdan gelen mesajların uzmanlık gerektirip gerektirmediği filtrelenerek `needsExpertReview` state'ine yansıtıldı. Böylece gereksiz uzman meşguliyeti engellenirken hayati konularda hızlı müdahale yolu açıldı.
