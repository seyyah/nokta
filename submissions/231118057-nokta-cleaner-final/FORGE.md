...
**Hafta 2 Track:** B  ·  **Hafta 3 Track:** 1
...
**Şart (Hafta 3):** ≥2 success cycle + ≥1 ROLLBACK loglu, dikte edilmiş raporlardan tetiklenmiş

> **Kayıt politikası:** Asla başarısız hipotez silinmeyecek — ROLLBACK satırı `success` ile aynı görünürlükte kalacak. Fake-backfill yok; bir satır gerçek koşulduktan sonra doldurulur.
>
...
> Cycle koşulduktan sonra **her satır gerçek değerlerle** doldurulacak. Boş bırakılan kolonlar `_TBD_`.

| # | Tarih | Rapor | Hipotez | Sonuç | Değişen dosyalar | Test | Commit | kg | Human touch |
|---|---|---|---|---|---|---|---|---|---|
| _Cycle 1_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ |
| _Cycle 2_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ |
| _Cycle 3_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ | _TBD_ |

**Hedef dağılım:** ≥2 satır `success`, ≥1 satır `rollback`. Ratchet (kg progression) monotonik artmaz da olur — Hafta 3 brief'i ratchet'i bağlayıcı yapmıyor, sadece rollback'in **bilinçli ve zekice** olmasını istiyor (cheap shot ≠ rollback).
...
- Hafta 3 dikte raporlarının hangi dosyaları hedeflediği önceden bilinmiyor — agent'ın `LOCATE` adımı için raporda en az 1 dosya/satır ipucu olmalı; yoksa human touch sayacı artar.
- Rollback'i bilinçli planla: native dep eklemek gibi cheap shot **bilinmiyor sayılır**. Hipotezin mantıken yanlış çıkması beklenir bir senaryo seç (örn. "X değişkeni Y'ye neden olur" — ölçtükten sonra hayır olmadığını gör).
- Dev server CI mode'dan çıkarılmış halde, HMR aktif; cycle sırasında edit yapınca browser refresh yetiyor — ama `.env` ya da `metro.config.js` değişirse server restart gerek.
