...
type Props = {
  navigation: StackNavigationProp<RootStackParamList, 'Spec'>;
...
export default function SpecOutputScreen({ navigation, route }: Props) {
  ...
  async function loadSpec() {
    ...
    try {
      ...
      setError(e.message ?? 'Spec oluşturulamadı.');
    } finally {
      setLoading(false);
    }
  }
  ...
  function renderSpec(text: string) {
    return text.split('\n').map((line, i) => {
      if (line.startsWith('## ')) return <Text key={i} style={styles.sectionHeader}>{line.replace('## ', '')}</Text>;
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.inner}>
        <View style={styles.headerRow}>
          ...
          {!loading && !error && (
            <TouchableOpacity style={styles.shareBtn} onPress={() => Share.share({ message: `NOKTA Spec\n\nFikir: ${idea}\n\n${spec}` })}>
        ...
        {error && (
          <View>
            ...
            <TouchableOpacity style={styles.humanBtn} onPress={() => Linking.openURL(`mailto:?subject=NOKTA%20Spec%20Yard%C4%B1m%C4%B1&body=Fikir%3A%20${encodeURIComponent(idea)}`)}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.startOverBtn} onPress={() => navigation.popToTop()}>
...
const styles = StyleSheet.create({
  ...
  shareBtn: { backgroundColor: '#6c47ff', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
  ...
  sectionHeader: { color: '#6c47ff', fontSize: 13, fontWeight: '700', letterSpacing: 2, textTransform: 'uppercase', marginTop: 18, marginBottom: 6 },
  ...
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 24, backgroundColor: '#0a0a0a', borderTopWidth: 1, borderTopColor: '#1a1a1a' },
  startOverBtn: { backgroundColor: '#1a1a1a', borderRadius: 12, paddingVertical: 14, alignItems: 'center', borderWidth: 1, borderColor: '#2a2a2a' },
