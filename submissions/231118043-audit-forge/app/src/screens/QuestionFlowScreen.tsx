...
type Props = {
  navigation: StackNavigationProp<RootStackParamList, 'Questions'>;
...
export default function QuestionFlowScreen({ navigation, route }: Props) {
  ...
  useEffect(() => { loadQuestion(); }, [qas.length]);
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.progressLabel}>Soru {questionIndex + 1} / {TOTAL_QUESTIONS}</Text>
        ...
        <View style={styles.questionBox}>
          {loading ? (
            ...
            <View>
              ...
              <TouchableOpacity style={styles.humanBtn} onPress={() => Linking.openURL(`mailto:?subject=NOKTA%20Yard%C4%B1m&body=Fikir%3A%20${encodeURIComponent(idea)}`)}>
        ...
        {!loading && !error && (
          <>
            ...
            />
            <TouchableOpacity style={[styles.nextBtn, !currentAnswer.trim() && styles.nextBtnDisabled]} onPress={handleNext} disabled={!currentAnswer.trim()}>
              <Text style={styles.nextBtnText}>{questionIndex + 1 < TOTAL_QUESTIONS ? 'Devam →' : 'Spec Oluştur ✦'}</Text>
            </TouchableOpacity>
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  ideaPill: { backgroundColor: '#111', borderRadius: 10, padding: 12, marginBottom: 28, borderWidth: 1, borderColor: '#222' },
  ...
  answerInput: { backgroundColor: '#1a1a1a', color: '#fff', borderRadius: 12, padding: 16, fontSize: 16, lineHeight: 24, minHeight: 110, borderWidth: 1, borderColor: '#2a2a2a', marginBottom: 20 },
  nextBtn: { backgroundColor: '#6c47ff', borderRadius: 12, paddingVertical: 16, alignItems: 'center' },
