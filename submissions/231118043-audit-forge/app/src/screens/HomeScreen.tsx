  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        />
        <Text style={[styles.charCount, idea.trim().length < 10 && idea.length > 0 ? styles.charCountWarn : null]}>
          {idea.trim().length < 10 && idea.length > 0 ? '⚠️ ' : ''}{idea.trim().length} karakter{idea.trim().length < 10 ? ' (min 10)' : ''}
        </Text>
        ...
        <Text style={styles.hint}>AI fikrni spec'e dönüştürmek için 5 mühendislik sorusu soracak.</Text>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: '#1a1a1a', color: '#fff', borderRadius: 12, padding: 16, fontSize: 16, lineHeight: 24, minHeight: 140, borderWidth: 1, borderColor: '#2a2a2a' },
  charCount: { color: '#555', fontSize: 12, marginTop: 6, textAlign: 'right' },
  charCountWarn: { color: '#ff6b6b' },
  button: { backgroundColor: '#6c47ff', borderRadius: 12, paddingVertical: 16, alignItems: 'center', marginTop: 24 },
