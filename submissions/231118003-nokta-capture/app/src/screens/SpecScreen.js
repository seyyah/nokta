    ...
    <LinearGradient colors={[COLORS.bg, "#1A0830"]} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.scoreRow}>
          <View style={styles.scoreBadge}><Text style={styles.scoreLabel}>Netlik</Text><Text style={styles.scoreVal}>{currentSpec.scores?.clarity || 0}/10</Text></View>
          <View style={styles.scoreBadge}><Text style={styles.scoreLabel}>Fizibilite</Text><Text style={styles.scoreVal}>{currentSpec.scores?.feasibility || 0}/10</Text></View>
          <View style={styles.scoreBadge}><Text style={styles.scoreLabel}>Etki</Text><Text style={styles.scoreVal}>{currentSpec.scores?.impact || 0}/10</Text></View>
        </View>

        {/* Ambiguity Detector */}
        {currentSpec.ambiguities && currentSpec.ambiguities.length > 0 && (
          <View style={styles.ambiguityBox}>
            <Text style={styles.sectionTitle}>⚡ Belirsizlik Dedektörü</Text>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "rgba(5,5,16,0.8)", padding: 24, borderRadius: 24, borderWidth: 1, borderColor: COLORS.mint, marginBottom: 20, shadowColor: COLORS.mint, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.4, shadowRadius: 10 },
  justification: { color: COLORS.pink, fontStyle: "italic", marginBottom: 20, backgroundColor: "rgba(255,0,127,0.1)", padding: 15, borderRadius: 10, borderWidth: 1, borderColor: "rgba(255,0,127,0.3)" },
  label: { fontSize: 13, color: COLORS.mint, fontWeight: "900", marginTop: 20, textTransform: "uppercase", letterSpacing: 2 },
  val: { fontSize: 16, color: "#FFFFFF", marginTop: 8, lineHeight: 24, textShadowColor: "rgba(255,255,255,0.3)", textShadowOffset: {width: 0, height: 0}, textShadowRadius: 5 },
