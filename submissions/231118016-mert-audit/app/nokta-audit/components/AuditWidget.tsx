...
export function AuditWidget({ deps, appName = 'App', initialPosition }: Props) {
  ...
  const handleExportMd = async () => {
    ...
    console.log('\n\n--- AŞAĞIDAKİ MARKDOWN METNİNİ KOPYALAYIN ---\n\n' + md + '\n\n-----------------------------------------------\n\n');
    const fileUri = await deps.writeFile(`bug-report-${stamp()}.md`, md);
    try {
      await deps.shareFile(fileUri);
    } catch(e) {
      console.warn('Web ortaminda share desteklenmeyebilir, konsoldan kopyalayabilirsiniz.');
    }
  };
  ...
  return (
    <>
      ...
      {mode === 'idle' && (
        <Animated.View
          style={[styles.fab, { left: pan.x, top: pan.y, transform: [{ translateX: -60 }], backgroundColor: '#2b6cb0' }]}
        >
          <TouchableOpacity style={{flex:1, alignItems:'center', justifyContent:'center', width:'100%'}} onPress={handleOpenList}>
