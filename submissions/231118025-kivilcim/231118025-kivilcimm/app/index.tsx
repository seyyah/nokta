...
import SlopMetre from '../src/components/SlopMetre';
import { analyzeSlopScore, analyzeProblemSolution } from '../src/services/aiService';
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (psResult.type === 'solution') {
                setFeedback({ type: 'solution', message: psResult.feedback });
                setLoading(false);
                return;
            }
            ...
            if (slopResult.score < 30) {
                setFeedback({ type: 'vague', message: slopResult.reason });
                setLoading(false);
                return;
            }
            router.push({
                pathname: '/flow',
                params: { idea: ideaText, initialScore: slopResult.score },
            });
        } catch (error) {
            setFeedback({ type: 'error', message: 'Bağlantı hatası: ' + error.message });
        } finally {
            setLoading(false);
        }
    };
        ...
        <View style={styles.container}>
            ...
            >
                <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
                    ...
                    <SlopMetre score={slopData.score} type={slopData.type} reason={slopData.reason} visible={slopVisible} />
                    {feedback && (
...
const styles = StyleSheet.create({
    ...
    hero: { alignItems: 'center', paddingTop: 20, paddingBottom: 16, paddingHorizontal: 30 },
    ...
    inputContainer: { marginHorizontal: 20, marginBottom: 8 },
    textInput: {
        backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.surfaceBorder,
        borderRadius: 16, padding: 16, color: Colors.text, fontSize: 16, minHeight: 140, maxHeight: 220, lineHeight: 24,
    },
    charCount: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', marginTop: 4, paddingRight: 4 },
    charText: { color: Colors.textMuted, fontSize: 12 },
    feedbackContainer: {
        flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginHorizontal: 20,
        marginTop: 12, padding: 14, borderRadius: 12, borderWidth: 1,
    },
    feedbackIcon: { fontSize: 18, marginTop: 1 },
    feedbackText: { color: Colors.text, fontSize: 14, lineHeight: 20, flex: 1 },
    tips: {
        marginHorizontal: 20, marginTop: 16, padding: 14, backgroundColor: Colors.surface,
        borderRadius: 12, borderWidth: 1, borderColor: Colors.surfaceBorder,
    },
    tipTitle: { color: Colors.textSecondary, fontSize: 13, fontWeight: '700', marginBottom: 8 },
    tipText: { color: Colors.textMuted, fontSize: 13, lineHeight: 20 },
    bottomBar: { paddingHorizontal: 20, paddingVertical: 12, borderTopWidth: 1, borderTopColor: Colors.surfaceBorder, backgroundColor: Colors.backgroundLight },
    submitButton: { backgroundColor: Colors.primary, paddingVertical: 16, borderRadius: 14, alignItems: 'center' },
    submitButtonDisabled: { backgroundColor: Colors.surfaceLight },
