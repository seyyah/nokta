...
REPAIR: Keep the recorder hook isolated, map dB to `0..1`, add attack/release smoothing, and pass the same amplitude into bars and avatar.

TEST: `npm run typecheck`; manual Android device test should verify permission, silence idle and speech response.

RESULT: TypeScript passes. Runtime mic/device verification remains a manual check because desktop typecheck cannot grant a phone microphone.
