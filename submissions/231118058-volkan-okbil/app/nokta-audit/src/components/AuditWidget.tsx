...
export function AuditWidget({ deps, appName = 'App', initialPosition }: Props) {
  ...
  const handleAutoFix = async () => {
    ...
    const md = buildMarkdown(all, { appName, exportedAt: new Date().toISOString(), totalNotes: all.length });
    try {
      Alert.alert("Otonom Onarım Başladı", "Lokal sunucuya (forge-server) istek gönderildi. Lütfen bekleyin...");
      // Using 10.0.2.2 for Android emulator, change to localhost for iOS simulator
      const host = typeof navigator !== 'undefined' && navigator.product === 'ReactNative' && require('react-native').Platform.OS === 'android' ? '10.0.2.2' : 'localhost';
      ...
      Alert.alert("Bağlantı Hatası", "Lokal sunucu (localhost:3000) çalışmıyor olabilir. Lütfen 'node forge-server.js' komutunu çalıştırın.");
    }
  };
